...
from lib.RUN import (
    create_service_tagging,
    create_service_preview_tagging,
...
async def sst_create(request: Request):
    ...
    if certi_status == "PROVISIONING":
        ...
        if entry_list != []:
            ...
            if (
                min_time_entry["store_id"] != "null"
                and min_time_entry["store_id"] != ""
                and min_time_entry["store_id"] != None
            ):
                print(
                    "The request initiates, shortly cloud run deployment process start"
                )
                ...
                preview_server_url, preview_name = await create_service_preview_tagging(
                    store_id, region, container_config
                )
                sample_set_iam_policy(preview_name)
                result1, tagging_name, details = await create_service_tagging(
                ...
                if domain != None:
                    ...
                    ssl_create_managed(
                        certificate_name=certificate_name, domains=list_domain
                    )
                    ...
                    hostrule_add(
                        domain=[domain],
                        backend_service_name=backend_service_name,
                        paths=["/test", "/dev", "/pre-prod"],
                    )
                    print("Hostrule added successfully")
                    print(
                        "All the creation process done.. IMP details store into datastore"
                    )
                else:
            ...
            print(
                "There are no requests left for creation in the datastore because datastore is empty"
            )
        ...
        if entry_list != []:
            ...
            if (
                ...
                json_data = {
                    ...
                    "preview_tagging_server_url": task.get(
                        "preview_tagging_server_url"
                    ),
                ...
                if container_config != None:
                    ...
                    store_id = min_time_entry["store_id"]
                    domain_datastore = str(json_data["domain"])
                    preview_tagging_server_url = str(
                        json_data["preview_tagging_server_url"]
                    )
                    ...
                    operation = run_client.update_service(
                        metadata=[("name", service_name)], request=request
                    )
                    ...
                    operation = run_client.update_service(
                        metadata=[("name", preview_service_name)],
                ...
                # Domain given and update
                    ...
                    parent_key = None
                    custom_key_update = client.key(
                        kind_update, str(store_id), parent=parent_key
                    )
            ...
            print(
                "There are no requests left for updation in the datastore because datstore is empty"
            )
